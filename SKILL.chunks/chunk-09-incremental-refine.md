...
AI 自主处理：
  ...
  5. ≥3次仍不合格 → 最终文档该处写⚠️提示：「本章质量未过盲审，请人工复核」
     → 不阻塞全局 DONE
```
...
| GAP 情况 | 阈值 | 决策 |
|---------|------|------|
| 核心模块 P0 功能缺失（应该有CRUD但缺U/D） | 核心模块 P0 ≥ 2个 | 触发「模块级回灌 L2→L3」对应模块 |
| 核心模块 P0 但非核心模块 P0 多 | 非核心 P0 ≥ 5 个 & 核心无P0 | 不回灌，文档中标注为「扩展功能」，不阻塞 |
...
| 字段覆盖 < 50%（核心ENTITY） | documented/fields_total < 50 | 回灌 L5 对应ENTITY的字段详情 |
| 以上都不严重 | completeness_score ≥ 70 | **不回灌**，直接推进 AUTO_REVIEW，置信度低的靠AI裁决，文档中加⚠️标注 |
...
- ⏭️ 跳过：数据字典模块 P0=缺2个不影响核心功能，加⚠️标注不回灌
