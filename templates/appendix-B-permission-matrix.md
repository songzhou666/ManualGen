...
- 单元格值定义：
  - ✅ = 可执行（源码中找到 2+ 条证据：路由 roles、@PreAuthorize、菜单 v-if 等）
  - ⚠️ = 推断可执行（只有 1 条证据或聚合推断，仍未达 ≥2）
  - ❌ = 不可执行（显式 hasRole 不包含 / 菜单 v-if = false / 后端 Deprecated 等）
  - ❔ = 无证据（AUTO_REVIEW 阶段仍无法判定，对应附录 C 中 HUMAN_REVIEW_REQUIRED 节点）
...
> ⚠️ 以上流转图为 Snake `SNAKE_0xx 权限晋升链路` 摘要，完整跨模块说明见附录 D。
