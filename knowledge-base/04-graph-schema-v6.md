...
> **铁律：图谱中没有证据的节点 = 不可靠 = WRITE阶段默认不采纳，除非 AUTO_REVIEW 阶段已裁决（补证据/推断）或列入附录 C 人工复核清单。**
...
### 证据不足时的处理
- 如果节点 `meta.confidence < 0.7`，在 AUTO_REVIEW 阶段按规则三选一裁决：① 补证据（局部精读源码）② 推断（WRITE 加警告）③ 列附录 C 人工复核清单。裁决明细写入 `_auto_decisions.md`：
 > "以下 N 项是 AI 推断的，未找到明确代码证据，已按 AUTO_REVIEW 规则处理，如需人工复核见附录 C：
 > 1. [功能] 客户合并 — 置信度 0.62，未找到明确的按钮/API → 推断
