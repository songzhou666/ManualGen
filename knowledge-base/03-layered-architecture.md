...
| G1: L1→L2 | 全部模块的L1都完成？或 核心模块先完成？ | 支持核心优先模式，先进入核心模块的L2 |
| G2: L2→L3 | 核心页面的区域划分完整 | 补充L2 |
| G3: L3→L4 | 每个核心区域≥3个功能点 | 补充功能识别 |
| G4: L4→L5 | 每个核心功能≥5步操作，含流程图 | 补充操作步骤 |
| G5: L5→WRITE | 字段覆盖率≥80%，权限矩阵完成度≥70% | 补充细节 |

**核心优先模式**：可以只对核心模块走完L0→L5，非核心模块先停在L2或L3，保证关键内容质量。
...
**每个图谱节点必须有来源证据**，无证据的节点视为"猜测"，在AUTO_REVIEW阶段自动裁决（补证据/推断⚠️/附录C待复核）并标记：
...
- 0.7~0.95：单源有明确代码证据
- 0.4~0.7：从上下文推断（AUTO_REVIEW阶段裁决：补证据/推断⚠️/附录C待复核，裁决写入 `_auto_decisions.md`）
