...
> 🤖 本手册由 ManualGen v6.1 全流程 AI 自主生成（无用户决策参与）。
> 置信度报告：overall_quality = {87} / 100。其中 ⚠️ 标记段落为 AI 推断或证据不足部分（约占 {11}%）。
...
1.4 手册使用说明（⚠️ 标记含义 + 章节导航 + 证据溯源方法说明）
...
> 共 AI 自主处理 {284} 项：
> - 低置信节点：合并/补证据 {112} 项，标记⚠️待复核 {21} 项
...
|------|--------|------|------|------|------|
| 客户管理 | ✅全部 | ✅查看/编辑 | ❌ | ✅查看 | ❌ |
| 订单管理 | ✅全部 | ✅创建/审核 | ✅查看 | ✅代客下单 | ✅发货 |
| ... | ... | ... | ... | ... | ... |

## B.3 功能-角色细目（纵表·逐项）
| FN_ID | 功能名 | 模块 | 管理员 | 运营 | 财务 | 客服 | 仓储 | 证据 |
|-------|--------|------|--------|------|------|------|------|------|
| FN_015 | 创建订单 | 订单管理 | ✅ | ✅ | ❌ | ✅(代客) | ❌ | 路由 meta.roles + 前端 @hasPermission('order:create') |
| FN_016 | 审核订单 | 订单管理 | ✅ | ✅ | ❌ | ❌ | ❌ | 路由 meta.roles + 后端 PreAuthorize('order:audit') ⚠️ |
| ... | ... | ... | ... | ... | ... | ... | ... | ... |
```
