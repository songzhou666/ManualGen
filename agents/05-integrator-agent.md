...
> 本手册由 ManualGen v6.2 全流程 AI 自主生成（无用户决策参与）。
> 置信度报告：overall_quality = {87} / 100。其中 标记段落为 AI 推断或证据不足部分（约占 {11}%）。
