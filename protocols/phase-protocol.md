...
| 4 | L3_FUNCTION | Skeleton + NodeWeaver | L2 regions | `_kb/L3_functions/*.json` | **全部区域 100% 覆盖**（批循环，`batches_done==batches_total`）+ 已识别 FUNCTION ≥90% 带 trigger_element + OPERATES_ON |
...
| 10 | RESOLVE | Resolver v6 | auto_review + graph | `_resolution.md` + 各 resolution 写回 graph | HIGH 冲突 100% 有决策（不论保守解或标记） |
...
| 15 | AUDIT | 主控（10维评分 + 第⑪维覆盖完整性硬门） | _integration.md + graph_quality | `_audit.md` + _audit-summary.md | 10 维 ≥ 60 **+ 第⑪维覆盖完整性硬性 PASS/FAIL**（L1_index 全模块 vs 手册章节逐一比对，未覆盖必须溯源到附录 F；不 PASS → 回退 GAP_ANALYSIS 补齐） |
| 16 | TODO_RESOLVE | 主控 AI 逐条解决 | audit.md | `_todo_resolution.md` + updated todo_list | P0 TODO 100% RESOLVED 或 BLOCKER 评估完成 |
| 17 | JUDGE | 子 Agent 盲审（模块级打回） | _judgment.md | `_judgment.md`（含 per_module_scores） | ≥70%模块 PASS；不合格且≤2次重试返回 WRITE；≥3次 DONE+ |
...
| 阶段 | 跳过条件 | 跳过处理 | 跳过产物 |
|------|----------|----------|----------|
| L1~L5 任一层 | baton.layers[Lx].status == "completed" **且 `batches_done == batches_total` 且无 `incomplete_batches`**（激活恢复） | 全部满足才直接推进下层；任一不满足 → **回退该层重做，不信任旧标记** | 标记复用 |
...
Master 在 L0 完成后判断：
 ...
 → 必须同步产出 `_kb/L1_index.json`（对照 _exploration 模块清单生成，供 AUDIT ⑪ 比对）
 → baton.meta.small_project_fastpath = true
 else:
 → 按 6 层正规流程，不允许跳层
```
**原因**：6 层流程虽完整，但小型项目开 6 层成本高。v5 流程对小项目足够。
**注意**：走捷径也必须产出 L1_index.json，否则 AUDIT ⑪ 覆盖完整性硬门无法执行（模块缺失会静默通过）。
...
阻断：
 以上 4 类都未处理而直接推进 → 阻断
注意：low_confidence 即使仍残留也不阻断（文档加 **警告** 就行），关键是 AI 必须"处理过"而非"放过"。
```
...
阻断：
 ...
 - _appendix/ 中 B、C、D、E、F 五个附录任意缺失 → 阻断（F 为未覆盖清单，core_priority 与 full 模式都必须产出）
...
**+ 第⑪维覆盖完整性硬门（PASS/FAIL，不计权重分）：L1_index 全模块 vs 手册章节比对，未覆盖必须溯源附录 F；不 PASS → 回退 GAP_ANALYSIS**
...
盲审按「模块级」评分（每模块 70 分合格，不看综合分）：
 ...
 → meta.state = WRITE（按 SKILL.chunk09 §三 只重写该模块）
 - 某模块 < 70 & 重试 ≥ 3 → 不再重写，最终文档该模块末尾加 **警告** 提示 + 附录 C 列成 Top 未决项
...
_reference_check.md (REFERENCE_CHECK)
 ↓
_integration.md + _appendix/B~F.md + {项目名称} 用户操作手册.md (INTEGRATE)
 ↓
_audit.md + _audit-summary.md (AUDIT · 10维+⑪硬门)
 ↓
...
若激活恢复时 baton.meta.state ∈ {L0..L5}:
 if baton.layers[Lx].batches_done == baton.layers[Lx].*_batches_total
    and baton.layers[Lx].quality_score ≥ 层阈值（L0≥80, L1≥75, L2≥70, L3≥65, L4≥65, L5≥60）
    and 无 baton.layers[Lx].incomplete_batches（未过质量门的批）:
 → 层已完成，推进下层
 elif baton.layers[Lx].batches_done < *_batches_total:
 → 从 batches_done + 1 开始跑下一批（不重跑已完成的批）
 elif 存在 incomplete_batches:
 → 存在未过质量门/未覆盖的批 → 先回灌补全，不得推进下层
 else:
 → 已完成但质量分不达标 → 回退该层重做（记入 rework.history），不信任旧标记
